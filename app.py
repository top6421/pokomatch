...
def init_state():
    if 'image_list' not in st.session_state:
        ...
        st.session_state.image_list = [f for f in os.listdir(IMAGE_FOLDER) if f.endswith('.png')]
...
if st.session_state.finished:
    st.success(f'게임 종료! 맞춘 개수: {st.session_state.score} / 50')
    if st.button('다시하기'):
    ...
    st.write(f'문제 {st.session_state.current_idx + 1} / 50')
    ...
    if st.session_state.answered:
        if st.session_state.current_idx < 49:
            if st.button('다음 문제'):
